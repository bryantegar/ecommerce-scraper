...
Lazada butuh cookie (login) untuk akses data.
...
TUBE_KEYWORD = "ecommerce_crawler_lazada_keyword"
TUBE_STORE   = "ecommerce_crawler_lazada_store"
TUBE_REVIEW  = "ecommerce_crawler_lazada_review"
...
class WorkerLazada(BaseWorker):
    def __init__(self, tube: str = TUBE_KEYWORD):
        ...
        logger.info(f"WorkerLazada siap. Tube={tube} MAX_PAGES={MAX_PAGES}")
    ...
    def process_job(self, job_data: dict) -> bool:
        ...
        try:
            ...
            final_status = "failed"
            error_msg    = str(e)
            logger.error(f"❌ Lazada job [{job_id}] error: {e}")
            raise
        finally:
            log_job_finish(job_id, final_status, total_saved, error_msg)

        logger.info(f"✅ Lazada '{name}': {total_saved} items saved, status={final_status}")
        return True

    def _process_pages(self, job_id, job_type, keyword, store_url, name, max_page, cookies):
        total = 0
        for page_num in range(1, max_page + 1):
            logger.info(f"  📄 Lazada {job_type} page {page_num}/{max_page}...")

            if job_type == "store":
                resp = self.service.scrape_lazada_store(store_url, page=page_num, cookies=cookies)
            else:
                resp = self.service.scrape_lazada_keyword(keyword, page=page_num, cookies=cookies)
            ...
            try:
                # Lazada punya struktur berbeda tergantung endpoint
                if job_type == "store":
                    items = resp_json.get("data", {}).get("listItems", [])
                else:
                    items = resp_json.get("listItems", [])
            ...
            saved = self.save_and_publish(
                items=items,
                save_fn=lambda **kw: save_lazada_items(resp_json=resp_json, **kw),
                job_id=job_id,
                name=name,
                job_type=job_type,
    ...
    def _process_review(self, job_id, product_url, max_page, cookies):
        total = 0
        for page_num in range(1, max_page + 1):
            logger.info(f"  💬 Lazada review page {page_num}/{max_page}...")

            resp = self.service.scrape_lazada_review(product_url, page=page_num, cookies=cookies)
            if resp is None or resp.status_code != 200:
                code = resp.status_code if resp else "None"
            ...
            try:
                reviews = resp_json.get("result", {}).get("reviews", [])
            except (AttributeError, TypeError):
...
if __name__ == "__main__":
    ...
    WorkerLazada(tube=tube_map[args.job_type]).run()
