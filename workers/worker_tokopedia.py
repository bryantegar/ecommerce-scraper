"""Worker Tokopedia — Keyword + Store"""
...
class WorkerTokopedia(BaseWorker):
    def __init__(self):
        ...
        logger.info(f"WorkerTokopedia siap. MAX_PAGES={MAX_PAGES}")

    @property
    def platform(self): return "tokopedia"

    @property
    def tube_name(self): return "ecommerce_crawler_tokopedia_keyword"
    ...
    def process_job(self, job_data: dict) -> bool:
        ...
        try:
            for page_num in range(1, max_page + 1):
                logger.info(f"  📄 Tokopedia page {page_num}/{max_page}...")
                ...
                if resp is None or resp.status_code != 200:
                    logger.warning(f"  ⚠️  Status {resp.status_code if resp else 'None'} di page {page_num}")
                    break
                ...
                try:
                    ...
                    logger.warning(f"  ⚠️  Tidak bisa parse response page {page_num}")
                    break

                # FIX: simpan langsung, tidak pakai lambda
                saved = save_tokopedia_items(
                    job_id=job_id,
        ...
        logger.info(f"✅ Tokopedia '{keyword}': {total_saved} items, status={final_status}")
        return True
