...
Blibli tidak butuh cookie (public API).
...
TUBE_KEYWORD = "ecommerce_crawler_blibli_keyword"
TUBE_STORE   = "ecommerce_crawler_blibli_store"
TUBE_REVIEW  = "ecommerce_crawler_blibli_review"
...
class WorkerBlibli(BaseWorker):
    def __init__(self, tube: str = TUBE_KEYWORD):
        ...
        logger.info(f"WorkerBlibli siap. Tube={tube} MAX_PAGES={MAX_PAGES}")
    ...
    def process_job(self, job_data: dict) -> bool:
        ...
        try:
            ...
            final_status = "failed"
            error_msg    = str(e)
            logger.error(f"❌ Blibli job [{job_id}] error: {e}")
            raise
        finally:
            log_job_finish(job_id, final_status, total_saved, error_msg)

        logger.info(f"✅ Blibli '{name}': {total_saved} items saved, status={final_status}")
        return True

    def _process_pages(self, job_id, job_type, keyword, store_url, name, max_page):
        total = 0
        for page_num in range(1, max_page + 1):
            logger.info(f"  📄 Blibli {job_type} page {page_num}/{max_page}...")
            ...
            try:
                # Blibli response structure: data.products atau data.content
                data = resp_json.get("data", {})
                items = data.get("products", data.get("content", []))
            ...
            saved = self.save_and_publish(
                items=items,
                save_fn=lambda **kw: save_blibli_items(resp_json=resp_json, **kw),
                job_id=job_id,
                name=name,
                job_type=job_type,
    ...
    def _process_review(self, job_id, product_url, max_page):
        total = 0
        for page_num in range(1, max_page + 1):
            logger.info(f"  💬 Blibli review page {page_num}/{max_page}...")

            resp = self.service.scrape_blibli_review(product_url, page=page_num)
            if resp is None or resp.status_code != 200:
                code = resp.status_code if resp else "None"
            ...
            try:
                reviews = resp_json.get("data", {}).get("content", [])
            except (AttributeError, TypeError):
...
if __name__ == "__main__":
    ...
    WorkerBlibli(tube=tube_map[args.job_type]).run()
