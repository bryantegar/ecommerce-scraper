from workers.worker_olx import WorkerOLX
import argparse
import sentry_sdk
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Worker OLX',
    ...
    set_tags({'process.social_media': 'olx',
              'process.name': mode})
    if mode == 'worker_olx_keyword':
        worker_olx = WorkerOLX(
            allowed_usage=args.allowed_usage,
            use_proxy=args.do_not_use_proxy)
        worker_olx.worker_scrape_keyword()
